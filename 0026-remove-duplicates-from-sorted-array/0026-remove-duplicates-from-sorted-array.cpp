...
public:
    int removeDuplicates(vector<int>& nums) {
        int p1 = 1; // a pointer that will replace items when the other pointer finds a new unique item
        ...
        return p1; // we move p when we find a new unique element so it should contain the number of unique elements + the first element in the array
    }
};