...
public:
    int magnificentSets(int n, vector<vector<int>> &edges) {
        ...
        for (int node = 0; node < n; node++) {
            distances[node] = computeMaxDistance(adjList, node, n);
        }
        ...
        for (int node = 0; node < n; node++) {
            ...
            totalGroups += computeGroups(adjList, node, distances, visited);
        }
...
private:
    ...
    int computeMaxDistance(vector<vector<int>> &adjList, int srcNode, int n) {
    ...
    int computeGroups(vector<vector<int>> &adjList, int node, vector<int> &distances, vector<bool> &visited) {
        int maxGroups = distances[node];
        visited[node] = true;
        for (int neighbor : adjList[node]) {
            if (visited[neighbor]) continue;
            maxGroups = max(maxGroups, computeGroups(adjList, neighbor, distances, visited));
        }
        return maxGroups;
    }
};
